...
<body>
	...
	<div class="container">
		...
		<form class="form" action="${pageContext.request.contextPath}/students" method="post">
				...
				<tr>
					...
					<td>
						<div class="form-check">
							<input class="form-check-input" type="checkbox" name="hobbies" id="swimmingCheckBox" value="Swimming" ${ fn:containsIgnoreCase(student.hobbies, "Swimming") ? 'checked' : '' } />
							<label class="form-check-label" for="swimmingCheckBox">Swimming</label>
						</div>
						<div class="form-check">
							<input class="form-check-input" type="checkbox" name="hobbies" id="singingCheckBox" value="Singing" ${ fn:containsIgnoreCase(student.hobbies, "Singing") ? 'checked' : '' } />
							<label class="form-check-label" for="singingCheckBox">Singing</label>
						</div>
						<div class="form-check">
							<input class="form-check-input" type="checkbox" name="hobbies" id="dancingCheckBox" value="Dancing" ${ fn:containsIgnoreCase(student.hobbies, "Dancing") ? 'checked' : '' } />
				...
				<tr>
					...
					<td>
						<select class="languages-multiple-select" multiple="multiple" name="languages" style="width: 100%">
							<option value="C" ${ fn:containsIgnoreCase(student.languages, 'C') ? 'selected' : '' }>C</option>
							<option value="Rust" ${ fn:containsIgnoreCase(student.languages, 'Rust') ? 'selected' : '' }>Rust</option>
							<option value="Java" ${ fn:containsIgnoreCase(student.languages, 'Java') ? 'selected' : '' }>Java</option>
							<option value="Python" ${ fn:containsIgnoreCase(student.languages, 'Python') ? 'selected' : '' }>Python</option>
							<option value="C#" ${ fn:containsIgnoreCase(student.languages, 'C#') ? 'selected' : '' }>C#</option>
						</select>
					</td>
				</tr>
